...
export function PortalSidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const { currentUser, currentTenant, availableTenants, availableUsers, switchTenant, switchUser, switchRoleTier } = useAuth();

  const roleBadgeInfo: Record<UserRole, { label: string; tier: string; color: string }> = {
    SUPER_ADMIN: { label: "Super Admin", tier: "Tier 0 (Main Boss)", color: "bg-purple-900/50 text-purple-200 border-purple-700" },
    OWNER: { label: "Shop Owner", tier: "Tier 1 (Treasury & Pocket)", color: "bg-amber-900/50 text-amber-200 border-amber-700" },
    BRANCH_MANAGER: { label: "Branch Manager", tier: "Tier 2 (Counter & KYC)", color: "bg-blue-900/50 text-blue-200 border-blue-700" },
    FIELD_RECOVERY: { label: "Recovery Officer", tier: "Tier 3 (Field & Routes)", color: "bg-emerald-900/50 text-emerald-200 border-emerald-700" },
    CUSTOMER: { label: "Customer / Kharedar", tier: "Tier 4 (Self-Service)", color: "bg-teal-900/50 text-teal-200 border-teal-700" },
  };
    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
      ...
      <div className="px-4 py-3 border-b border-slate-800/80 bg-slate-950/40">
        ...
        <div className="relative">
          <select
            ...
            className="w-full bg-slate-800 text-xs font-semibold text-white border border-slate-700 rounded-lg px-2.5 py-1.5 appearance-none focus:outline-none focus:border-emerald-500 pr-7"
          >
      ...
      <div className="p-3 sm:p-4 bg-slate-950/70 border-b border-slate-800">
        ...
        <select
          ...
          className="w-full bg-slate-800 text-xs font-bold text-amber-300 border border-slate-700 rounded-lg px-2 py-1.5 focus:outline-none"
        >
          {availableUsers.map((u) => (
            <option key={u.id} value={u.id}>
              {roleBadgeInfo[u.role].tier} — {u.name}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="p-3 sm:p-4 border-t border-slate-800 bg-slate-950/40">
        ...
        >
          ...
          <span className="text-[10px] text-emerald-400 font-bold">Exit</span>
        </Link>
      </div>
    </div>
  );
